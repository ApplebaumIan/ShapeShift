...
class PDFDrawingTools{
	...
	let undoButton: UIButton = {
        ...
        let undoIcon = #imageLiteral(resourceName: "Undo").resized(newSize: CGSize(width: 40, height: 40))

			button.setImage(undoIcon, for: .normal )

//        button.heightAnchor.constraint(equalToConstant: undoIcon.size.height).isActive = true
//               button.widthAnchor.constraint(equalToConstant: undoIcon.size.width).isActive = true
//
...
//		button.setImage(#imageLiteral(resourceName: "cancel_button.png"), for: .normal)
		let resized = #imageLiteral(resourceName: "cancel_button").resized(newSize: CGSize(width: 40, height: 40))
		button.setImage(resized, for: .normal)
		//button.setTitle("clear", for: .normal)
		//button.titleLabel?.font = .boldSystemFont(ofSize: 24)
//        button.heightAnchor.constraint(equalToConstant: resized.size.height).isActive = true
//               button.widthAnchor.constraint(equalToConstant: resized.size.width).isActive = true
	...
	let colorButton: UIButton = {
		...
		let penResize = pen.resized(newSize: CGSize(width: 40, height: 40))
		button.setImage(penResize, for: .normal)
		let resized = image.resized(newSize: CGSize(width: 40, height: 40))
...
//
	...
	fileprivate func hideALLTools(_ stackView: UIStackView) {
		let colorButton = stackView.arrangedSubviews[0]
		let redButtonView = stackView.arrangedSubviews[1]
		let blueButtonView = stackView.arrangedSubviews[2]
		let blackButtonView = stackView.arrangedSubviews[3]
		let yellowHighlighterView = stackView.arrangedSubviews[4]
	...
	func layOutButtons() ->UIStackView {
		...
		if pdfViewDelagate!.document?.allowsCommenting == false {
			print("OOPS WRITE PROTECTED!!!!!!")
			writeProtected()
			hideALLTools(stackView)
		}else{
...
//		stackView.topAnchor.constraint(equalTo: viewToAddTools!.topAnchor, constant: 200).isActive = true
		...
		stackView.directionalLayoutMargins = NSDirectionalEdgeInsets(top: 3, leading: 0, bottom: -3, trailing: 0)
		return stackView
	}
	/*
	...
	*/
	...
	//		takeScreenshotButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 18)
		...
		let resized = #imageLiteral(resourceName: "screen_grab2.png").resized(newSize: CGSize(width: 40, height: 40))
	...
	func writeProtected() {
		print("PDF is read only")
		snackbar.show(message: "PDF is read only.",duration: 3)
//		hideALLTools(markupButtondDel!)
	}
